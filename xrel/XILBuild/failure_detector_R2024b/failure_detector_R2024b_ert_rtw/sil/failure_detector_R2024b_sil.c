/*
 * failure_detector_R2024b_sil.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: failure_detector_R2024b
 * Component Simulink Path: failure_detector_R2024b
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                failure_detector_R2024b_sil
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("failure_detector_R2024b");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
static void mdlInitializeSizes(SimStruct *S)
{
...

  {
    ...
    /* Use TLC with accelerator _ONLY_ for model reference sim target   */
    /* Model ref accelerator simulation of the cross release block is   */
    /* NOT supported, but we need to be able to build a stub SIM-target */
    if ssRTWGenIsModelReferenceSimTarget(S) {
      options |= SS_OPTION_USE_TLC_WITH_ACCELERATOR;
    }                                  /* if */
  ...
  if (!ssIsFirstInitCond(S)) {
    {
      void * pXILUtils = (void *) ssGetPWorkValue(S, 6);
      mxArray * rhs[ 2 ];
      rhs[0] = mxCreateString("PIL:xrelimport:UnsupportedResetStates");
